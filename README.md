...
A fasta file containing the consensus sequence of a sample. The consensus sequence is generated by applying the SNPs found in *snpbac_snps.recode.vcf* to the reference genome (this step is performed prior to running Gubbins, so regions of recombination have not been filtered out). These files are concatenated and used as input to Gubbins.
